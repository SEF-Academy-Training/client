...
import { UserSidebarData } from '../../DummyData/DummyData';
import { useTranslation } from 'react-i18next';
import { Link, useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { setActiveLink, setLogout } from '../../../Redux/Reducers/UserSlice';
...
const UserDashboardSideBar = () => {
    ...
    const DataLang = UserSidebarData.map((data) => {
        if (i18n.language === 'ar') {
            return ({
                'id': data.id,
                'title': data.titleAr,
    ...
    const handleSelectTitle = (title) => {
        { title == 'settings' && navigate('/settingsdashboard') }
    }
